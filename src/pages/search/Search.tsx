    ...
    <Helmet title="Search" />
      ...
      <Query query={search} variables={{
          ...
          {({ loading, error, data }) => {
            ...
            const groups = groupBy(get(data, '_allDocuments.edges', []), 'node.__typename');
            ...
            return (
              ...
              >
                {Object.keys(groups).map(key => (
                  <Group key={key} title={key === 'Article' ? 'Articles' : 'Pages'}>
                    {groups[key].map(({ node }) => (
                      <Item
                        key={node._meta.uid}
                        to={linkResolver(node)}
                        title={RichText.asText(get(node, 'title', []))}
                        text={RichText.render(node.short_description || node.text || node.description_seo || [], linkResolver)}
                      />
                    ))}
                  </Group>
                ))}
